...
# Get assets and temp file paths
ASSETS_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'assets')
TEMP_DIR = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))
...
def open_window(file_name, original_grayscale, RGB_full_image, RGB_color_palette, RGB_image_dimensions): 
    ...
    def decompress_rgb(byte_data, ColorPalette):
        imageData = Image.new('RGB', (RGB_image_dimensions.shape[1], RGB_image_dimensions.shape[0]), "black")
        ...
        try:
            while position < len(byte_data):
                ...
                if ((Byte & 0xC0) == 0xC0 and position < len(byte_data)):
                    runlength = Byte & 0x3F
                    if position < len(byte_data):
                        runvalue = byte_data[position]
                        position += 1
            ...
            imageData.save(os.path.join(TEMP_DIR, "decompressed.png"))
    ...
    def decompress_grayscale(compressed_data_grayscale, grayscale_color_palette, image_shape):  
        ...
        while(i < int(len(compressed_data_grayscale))):
            ...
            if image_pixel == 0:
                ...
                if (i < int(len(compressed_data_grayscale))-1):
                    i = i + 1
                    count = compressed_data_grayscale[i]

            for j in range(len(grayscale_color_palette)):
                if image_pixel == j:
                    palette_color = grayscale_color_palette[j]
                    break
            
            for m in range(count):
                decompressed_data_grayscale.append(palette_color)

            if(i < int(len(original_grayscale_image))):
                i = i + 1
            else:
                break
        
        decompressed_data_grayscale = np.array(decompressed_data_grayscale)
        decompressed_data_grayscale = decompressed_data_grayscale.reshape(image_shape[0], image_shape[1])
        cv2.imwrite(os.path.join(TEMP_DIR, "decompressed.png"), decompressed_data_grayscale)
    ...
    def transformation(transform_name, transform_image, transformation, open_image):
        ...
        window[transform_image].update(data = ImageTk.PhotoImage(image))
    
    def clear_info(grayscale):
        cv2.imwrite(os.path.join(TEMP_DIR, 'transformation.png'), grayscale)
        transformation("first", "-first_image-", "Original Image:", os.path.join(TEMP_DIR, 'transformation.png'))
    ...
    def unsharp_masked_image(k):
        unsharp_masking = original_image + k * mask
        unsharp_masking = np.clip(unsharp_masking, 0, 1)
        unsharp_masking[:] = unsharp_masking[:] * 255
        cv2.imwrite(os.path.join(TEMP_DIR, 'transformation.png'), unsharp_masking)

    def contraharmonic(Q):
        original_image = np.array(grayscale) / 255      
        kernel = np.array([ [1, 1, 1],          
                            [1, 1, 1],
                            [1, 1, 1] ])
        output = apply_power_filter(original_image, Q, kernel)
        return (output * 255)
    ...
    filter_column = [
        ...
        [sg.Button('Gradient', key = 'gradient', pad = ((5, 0), (10, 10)), border_width = 1, tooltip=" Apply Gradient ")],
    ...
    second_column = [
        ...
        [sg.Image(key="-first_image-", pad = ((0, 0), (0, 10)), size = (320, 240), filename=os.path.join(ASSETS_DIR, "empty.png"))],
        [sg.Text("Third Image:", key='third', size = (50, 1), text_color = "yellow", justification = 'center')],     
        [sg.Image(key="-third_image-", size = (320, 240), filename=os.path.join(ASSETS_DIR, "empty.png"))],   
    ]

    third_column = [
        [sg.Text("Second Image:", key='second', size = (50, 1), pad=((0, 0), (0, 8)), text_color = "yellow", justification = 'center')],     
        [sg.Image(key="-second_image-", pad = ((0, 0), (0, 10)), size = (320, 240), filename=os.path.join(ASSETS_DIR, "empty.png"))],  
        [sg.Text("Fourth Image:", key='fourth', size = (50, 1), text_color = "yellow", justification = 'center')],    
        [sg.Image(key="-fourth_image-", pad = ((0, 0), (0, 0)), size = (320, 240), filename=os.path.join(ASSETS_DIR, "empty.png"))],   
    ]

    layout = [
        [
            sg.Column(filter_column, vertical_alignment='center', p = ((0, 3), (60, 60))),
            sg.VSeperator(),
            sg.Column(second_column, element_justification = "center", pad = ((0, 0), (8, 25)), expand_y= True),
            sg.Column(third_column, element_justification = "center", pad = ((0, 0), (8, 25)), expand_y= True),
        ]
    ]
    ...
    while True:
        ...
        if event == "Exit" or event == sg.WIN_CLOSED:
            ...
            cv2.imwrite(os.path.join(TEMP_DIR, 'transformation.png'), original_grayscale)
            grayscale = np.array(original_grayscale)
            transformation("first", "-first_image-", "Original Image:", os.path.join(TEMP_DIR, 'transformation.png'))     # display the gamma transformed image 
            ...
            cv2.imwrite(os.path.join(TEMP_DIR, 'transformation.png'), grayscale)
            transformation("first", "-first_image-", "Original Image:", os.path.join(TEMP_DIR, 'transformation.png'))     # display the image
            ...
            cv2.imwrite(os.path.join(TEMP_DIR, 'transformation.png'), weightAverage)
            transformation("second", "-second_image-", "Weighted Average Filter:", os.path.join(TEMP_DIR, 'transformation.png'))     # display the filtered image 
            ...
            cv2.imwrite(os.path.join(TEMP_DIR, 'transformation.png'), medianFilter)
            transformation("third", "-third_image-", "Median Square Filter:", os.path.join(TEMP_DIR, 'transformation.png'))     # display the filtered image 
            ...
            cv2.imwrite(os.path.join(TEMP_DIR, 'transformation.png'), medianFilter)
            transformation("fourth", "-fourth_image-", "Median Cross Filter:", os.path.join(TEMP_DIR, 'transformation.png'))     # display the filtered image 
            ...
            cv2.imwrite(os.path.join(TEMP_DIR, 'transformation.png'), sharpenedImage)
            transformation("second", "-second_image-", "Laplacian Filter in Frequency Domain:", os.path.join(TEMP_DIR, 'transformation.png'))     # display the filtered image 
            ...
            cv2.imwrite(os.path.join(TEMP_DIR, 'transformation.png'), sharpenedImage)
            transformation("third", "-third_image-", "Laplacian Filter in Spatial Domain:", os.path.join(TEMP_DIR, 'transformation.png'))     # display the filtered image
            ...
            transformation("second", "-second_image-", "Unsharp Masking:", os.path.join(TEMP_DIR, 'transformation.png'))     # display the filtered image 
            ...
            transformation("second", "-second_image-", "Highboost Filtering (k="+str(k)+"):", os.path.join(TEMP_DIR, 'transformation.png'))     # display the filtered image 

            k=5
            unsharp_masked_image(k)
            transformation("third", "-third_image-", "Highboost Filtering (k="+str(k)+"):", os.path.join(TEMP_DIR, 'transformation.png'))     # display the filtered image

            k=10
            unsharp_masked_image(k)
            transformation("fourth", "-fourth_image-", "Highboost Filtering (k="+str(k)+"):", os.path.join(TEMP_DIR, 'transformation.png'))     # display the filtered image 
            ...
            plt.imsave(os.path.join(TEMP_DIR, 'transformation.png'), sobel_x_gradient, cmap='gray')
            transformation("second", "-second_image-", "Sobel X-Gradient:", os.path.join(TEMP_DIR, 'transformation.png'))     # display the sobel x-gradient image

            plt.imsave(os.path.join(TEMP_DIR, 'transformation.png'), sobel_y_gradient, cmap='gray')
            transformation("third", "-third_image-", "Sobel Y-Gradient:", os.path.join(TEMP_DIR, 'transformation.png'))     # display the sobel y-gradient image

            plt.imsave(os.path.join(TEMP_DIR, 'transformation.png'), sobel_filtered_image, cmap='gray')
            transformation("fourth", "-fourth_image-", "Sobel Magnitude:", os.path.join(TEMP_DIR, 'transformation.png'))     # display the sobel magnitude
            ...
            cv2.imwrite(os.path.join(TEMP_DIR, 'transformation.png'), contraharmonic(Q))
            transformation("second", "-second_image-", "Contraharmonic Mean Filter (Q="+str(Q)+"):", os.path.join(TEMP_DIR, 'transformation.png'))     # display the filtered image 
            ...
            cv2.imwrite(os.path.join(TEMP_DIR, 'transformation.png'), maxFilter)
            transformation("second", "-second_image-", "Max Filter:", os.path.join(TEMP_DIR, 'transformation.png'))     # display the filtered image 
            ...
            cv2.imwrite(os.path.join(TEMP_DIR, 'transformation.png'), minFilter)
            transformation("second", "-second_image-", "Min Filter:", os.path.join(TEMP_DIR, 'transformation.png'))     # display the filtered image 
            ...
            if (total_bytes_from_compression > original_image_in_bytes):
                ...
                transformation("second", "-second_image-", "Decompressed Image:", os.path.join(TEMP_DIR, "decompressed.png"))     # display the decompressed image  
            ...
            if (file_name.split(".")[1] != "pcx"):
                ...
                full_image = cv2.imread(os.path.join(TEMP_DIR, "tmp.png"))   # cv2.imread() returns a BGR (Blue-Green-Red) array
            ...
            if (total_bytes_from_compression > original_image_in_bytes or compressed_data_in_bytes <= 0):
                ...
                transformation("first", "-first_image-", "Original Image:", os.path.join(TEMP_DIR, "tmp.png"))     
                transformation("second", "-second_image-", "Decompressed Image:", os.path.join(TEMP_DIR, "decompressed.png"))     
